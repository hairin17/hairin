...
transformed parameters {
  ...
  for (s in 1:nSubject) {
    ...
    for (t in 1:(nTrials - nCues)){
      ...
      expectMu[cueIndexVec[t,s],s] = ((sigmaEpsilon + sigmaPsi) * expectMu[t,s] + (expectVar[t,s] * noxInput[t,s])) / 
                                       (sigmaEpsilon + sigmaPsi + expectVar[t,s]) ;
      expectVar[cueIndexVec[t,s],s] = ((sigmaEpsilon + sigmaPsi) * expectVar[t,s] / (sigmaEpsilon + sigmaPsi + expectVar[t,s])) + sigmaEta[s];
    }
